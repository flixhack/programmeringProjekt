
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title> Mellow </title>
    <link href="interface.css" rel="stylesheet" type="text/css">
    <link rel="icon" href="IMG/icon.png">
  </head>

<body>
<img src="IMG/Mellow2.png" alt="Title" style="width:190px;height:40px;">
<p class="sansserif">Hello, welcome to our site. <br /> This is the project for our programming exam.</p>
  </body>


  <input type="password" style="position:absolute;TOP:35px;left:1030px" name="fname"><br>
  <input type="email" style="position:absolute;TOP:35px;left:850px" name="fname"><br>
  <input type="image" src="login.png" alt="login" style="position:absolute;TOP:35px;left:1220px;WIDTH:32;HEIGHT:32">

  <A HREF="Registre.php" style="position:absolute;TOP:60px;left:850px">Not user yet? Registre here!</A>
</html>
